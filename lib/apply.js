exports.Apply = function(databaseUrl) {
  ...
  var jobString = fs.readFileSync('lib/schema/job.schema', 'utf8');
  var applicationString = fs.readFileSync('lib/schema/application.schema', 'utf8');

  var emailConfig = JSON.parse(fs.readFileSync('lib/email/config.json', 'utf8'));
      ...
      db.jobs.findOne({_id:mongojs.ObjectId(application.jobId)},function(err, job) {
        db.applications.insert(application,function(err, application) {
          if(err){
            ...
            console.log("Setting applicationId: "+application[0]._id)
            mailOptions.text="ApplicationId: "+application[0]._id
